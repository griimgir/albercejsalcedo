...
int* bubbleSort(int arr[], int n){
    ...
    int blop = n - 1; //this is the termination
    for(i = 0; i < blop; i++){
        for(j = 0; j < blop; j++){
            if(*(s_arr + 1) > *(s_arr + j + 1)){
                temp = *(s_arr + j + 1); //saves to temp to be ready to swapped
                *(s_arr + j + 1) = *(s_arr + j); //swap
                *(j + s_arr) = temp;    //store original to swapped positions spot
            }
        }
    }
    return s_arr;
...
int bSearch(int *arr, int a, int b, int key){
    ...
    if(a <= b){
        int middle = a + (b - a)/2; //This statement right here is good practice, i learned this middle initiation from merge sort.
        if(*(arr + middle) == key){
            return middle;
        }
