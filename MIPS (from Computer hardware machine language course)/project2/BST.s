.data 
# Worked with Elyse Geimer and Christopher Ahrens
...
loop:
        ...
        #t4 is 32 bits
        addi $v0, $zero, 11
        addi $v0, $zero, 11      #recieved help for this one
...
break:
        addi $v0, $zero, 4 #increment by 1* 1 = 4bytes
        la $a0, arrayList   
        jr $ra
...
inSort:
        ...
        addi $t1, $zero, 1 #sets t1 = 1
        addi $t2, $zero, 0 #set a new t2 = 0, this is for t1 -> t2 later. Transfering
...
BSTSearch:
        ...
        li $s0, 0 #beggining of the array
        li $t5, 1
        add $s3, $s0, $t5
...
jumpAddy:
        ...
        bgt $t2, $a3, Bleft # if the middle is less than the key than we traverse through the BST 
        #-------# Bright
        li $t5, 1
        add $s0, $t1, $t5
...
captured:
        li $t5, 1
        add $v0, $zero, $t5 #terminates loop and returns 1 to indicate it is found
        j exit
...
exitOLD:    	# terminate/end
    		li $v0, 10
    		syscall #this exit doesn't tell you if key is foun d or not
