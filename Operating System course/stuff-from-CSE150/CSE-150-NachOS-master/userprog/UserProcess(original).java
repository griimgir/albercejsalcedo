...
//	FilesArray[1].file = UserKernel.console.openForWriting();
	...
	int addOffset = Processor.pageFromAddress(vaddr);
	
	TranslationEntry satan = null;
	satan = pageTable[vpn];
	satan.used = true;
...
protected boolean loadSections() {
	...
	for (int s=0; s<coff.getNumSections(); s++) {
		...
		TranslationEntry satan = pageTable[vpn];
		satan.readOnly = section.isReadOnly();
		int ppn = satan.ppn;
...
//	
	...
	for(; Index < numFileDesc; Index++){
		...
		*/
		if(FilesArray[Index].file == null || FilesArray[Index].file_name == name){
...
//	}
  ...
  private int handleClose(int Index){
	...
	FilesArray[Index].file_name = "";
	 ...
	 if(FilesArray[Index].unlinked == true){
		if(UserKernel.fileSystem.remove(FilesArray[Index].file_name)){
			bread = 0; 
		 }
		 else{
			bread = -1;
  ...
  private int handleJoin(int cid, int status_addr){
	  ...
	  if(child == null){
		return -2; //child does not exist (exited)	  
	  }
	  ...
	  int test =writeVirtualMemory(status_addr, ByteIntConversion);
  ...
  private void handleExit(int STATUS){
	  ...
	  // remove children
	  Iterator<Integer> currID = this.CID.iterator();
	  while(currID.hasNext() && !CID.isEmpty()){
		int curr_ID = currID.next();
			currID.remove();
		  	UserProcess childprocess = getProcess(curr_ID);
		  	childprocess.ParentID = 900;
	  }
    ...
    protected final int numFileDesc = 20; //our chosen size for amount of files a process can have open
